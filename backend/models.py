...
class Portfolio(Base):
    ...
    hold_amount: Mapped[float] = mapped_column(Float, nullable=True)  # 持有金额
    day_pnl: Mapped[float] = mapped_column(Float, nullable=True)  # 当日盈亏
    day_pnl_pct: Mapped[float] = mapped_column(Float, nullable=True)  # 当日盈亏率
    hold_pnl: Mapped[float] = mapped_column(Float, nullable=True)  # 持有盈亏
    hold_pnl_pct: Mapped[float] = mapped_column(Float, nullable=True)  # 持有盈亏率
    total_pnl: Mapped[float] = mapped_column(Float, nullable=True)  # 累计盈亏
    total_pnl_pct: Mapped[float] = mapped_column(Float, nullable=True)  # 累计盈亏率
    week_pnl: Mapped[float] = mapped_column(Float, nullable=True)  # 本周盈亏
    month_pnl: Mapped[float] = mapped_column(Float, nullable=True)  # 本月盈亏
    year_pnl: Mapped[float] = mapped_column(Float, nullable=True)  # 今年盈亏
    position_pct: Mapped[float] = mapped_column(Float, nullable=True)  # 仓位占比
    hold_quantity: Mapped[float] = mapped_column(Float, nullable=True)  # 持有数量
    hold_days: Mapped[int] = mapped_column(Integer, nullable=True)  # 持仓天数
    latest_change_pct: Mapped[float] = mapped_column(Float, nullable=True)  # 最新涨幅
    latest_price: Mapped[float] = mapped_column(Float, nullable=True)  # 最新价
    unit_cost: Mapped[float] = mapped_column(Float, nullable=True)  # 单位成本
    breakeven_pct: Mapped[float] = mapped_column(Float, nullable=True)  # 回本涨幅
    month_1_pct: Mapped[float] = mapped_column(Float, nullable=True)  # 近1月涨幅
    month_3_pct: Mapped[float] = mapped_column(Float, nullable=True)  # 近3月涨幅
    month_6_pct: Mapped[float] = mapped_column(Float, nullable=True)  # 近6月涨幅
    year_1_pct: Mapped[float] = mapped_column(Float, nullable=True)  # 近1年涨幅
