...
- **11 维度技术指标评分** — 6 主指标 (MA / MACD / RSI / KDJ / BOLL / Volume) + 5 辅指标 (ATR / CCI / WR / SAR / ROC)
...
1. **金融数据精度优化**
   - 将所有Float类型改为Numeric(18,4)，确保金额计算精确

2. **API版本控制**
   - 为所有接口添加/api/v1/前缀，便于后续版本管理

3. **配置管理改进**
   - 从文件迁移到数据库存储，提供更好的可扩展性和多进程安全
   - 保持向后兼容，自动从旧配置文件迁移
