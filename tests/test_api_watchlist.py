...
def cleanup_watchlist(client):
    ...
    for item in resp.json():
        client.delete(f"/api/watchlist/{item['code']}")
    # 清理额外分组
    groups_resp = client.get("/api/watchlist/groups")
    for group in groups_resp.json():
        if group != "默认":
            client.delete(f"/api/watchlist/group/{group}")
...
class TestGetWatchlist:
    ...
    def test_list_after_add(self, client):
        """添加后列表应包含该股票"""
        client.post("/api/watchlist", json={"code": "600519", "name": "贵州茅台", "group": "默认"})
        resp = client.get("/api/watchlist")
        data = resp.json()
        assert len(data) == 1
    ...
    def test_filter_by_group(self, client):
        """按分组筛选应只返回该分组的股票"""
        client.post("/api/watchlist", json={"code": "600519", "name": "贵州茅台", "group": "白酒"})
        client.post("/api/watchlist", json={"code": "000001", "name": "平安银行", "group": "银行"})

        resp_baijiu = client.get("/api/watchlist", params={"group": "白酒"})
        assert resp_baijiu.status_code == 200
        data = resp_baijiu.json()
        assert len(data) == 1
        assert data[0]["code"] == "600519"

        resp_bank = client.get("/api/watchlist", params={"group": "银行"})
        assert resp_bank.status_code == 200
        data = resp_bank.json()
        assert len(data) == 1
        assert data[0]["code"] == "000001"

        resp_all = client.get("/api/watchlist")
        assert len(resp_all.json()) == 2

    def test_filter_nonexistent_group(self, client):
        """筛选不存在的分组应返回空列表"""
        client.post("/api/watchlist", json={"code": "600519", "name": "贵州茅台", "group": "默认"})
        resp = client.get("/api/watchlist", params={"group": "不存在的分组"})
        assert resp.status_code == 200
        assert resp.json() == []
...
class TestAddToWatchlist:
    ...
    def test_duplicate_detection(self, client):
        ...
        resp1 = client.post("/api/watchlist", json=payload)
        assert resp1.status_code == 200
        assert resp1.json()["message"] == "添加成功"

        resp2 = client.post("/api/watchlist", json=payload)
    ...
    def test_add_multiple_stocks(self, client):
        ...
        for code, name in [("600519", "贵州茅台"), ("000001", "平安银行"), ("000858", "五粮液")]:
            resp = client.post("/api/watchlist", json={"code": code, "name": name})
            assert resp.status_code == 200
...
class TestWatchlistGroups:
    ...
    def test_groups_after_adding(self, client):
        ...
        client.post("/api/watchlist", json={"code": "600519", "name": "贵州茅台", "group": "白酒"})
        client.post("/api/watchlist", json={"code": "000001", "name": "平安银行", "group": "银行"})
        client.post("/api/watchlist", json={"code": "000858", "name": "五粮液", "group": "白酒"})
...
class TestCreateGroup:
    ...
    def test_create_new_group(self, client):
        """创建新分组应成功"""
        resp = client.post("/api/watchlist/group", json={"name": "科技"})
        assert resp.status_code == 200
        assert "创建成功" in resp.json()["message"]

    def test_create_duplicate_group(self, client):
        """创建已存在的分组应返回已存在提示"""
        client.post("/api/watchlist/group", json={"name": "科技"})
        resp = client.post("/api/watchlist/group", json={"name": "科技"})
        assert resp.status_code == 200
...
class TestDeleteGroup:
    ...
    def test_delete_group_moves_items_to_default(self, client):
        ...
        resp = client.delete("/api/watchlist/group/白酒")
        assert resp.status_code == 200
    ...
    def test_delete_nonexistent_group_404(self, client):
        """删除不存在的分组应返回 404"""
        resp = client.delete("/api/watchlist/group/不存在的分组")
        assert resp.status_code == 404

    def test_delete_default_group_400(self, client):
        """删除默认分组应返回 400"""
        resp = client.delete("/api/watchlist/group/默认")
